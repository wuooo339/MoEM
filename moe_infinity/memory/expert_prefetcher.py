...
class ExpertPrefetcher(object):
    ...
    def prefetch_experts_test(self, fetch_layer_id, expert_matrix, k=1):
        ...
        for i in range(fetch_layer_id, min(fetch_layer_id + k, self.num_layers)):
            ...
            for j in range(self.num_experts):
                current_layer_experts.append((i, j, expert_matrix[i, j],self.expert_tensor_map[(i, j)]))
            # 按权重排序取Top32
            ordered_experts = sorted(current_layer_experts, key=lambda x: x[2], reverse=True)[0:32]
            # ==================== 打印专家权重 ====================
            print("\033[90mTop 20 Experts:\033[0m")  # 灰色小字注释
            for idx, (layer, expert, weight,tensor) in enumerate(ordered_experts, 1):
                if idx > 20:
                    add_prefetched_expert(layer, expert)
                    tensor_ids.append(tensor)
                color = "\033[32m" if weight > 0.5 else "\033[33m"  # 高权重绿色，低权重黄色
                print(f"{color}Expert {expert:2d}: {weight:.3f}({tensor})\033[0m", end=" | ")
                if idx % 4 == 0:  # 每行显示4个专家
                    print()
            print("\n" + "-" * 60)
